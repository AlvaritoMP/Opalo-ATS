...
export function lookupTallyValue(
    index: TallyFieldsIndex,
    tallyFieldRef: string,
    allowReuse = false
): string {
    ...
    for (const c of candidates) {
        if (index.byRef[c] !== undefined && index.byRef[c] !== '') {
            if (allowReuse || !isRefUsed(index, c)) {
...
export function parseIntegrationFieldMapping(integration: {
    ...
    try {
        ...
        if (raw && typeof raw === 'object' && !Array.isArray(raw)) {
            return normalizeTallyFieldMapping(raw as FieldMapping);
        }
    } catch {
        /* ignore */
    }
...
function assignStandardField(target: TallyCandidateInsert, key: string, value: string): void {
    ...
    switch (key) {
        ...
        case 'apellidoPaterno':
            target.apellido_paterno = value;
            break;
        case 'apellido_materno':
        case 'apellidoMaterno':
            target.apellido_materno = value;
            break;
        case 'name':
            target.name = value;
            break;
