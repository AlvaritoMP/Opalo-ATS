...
/**
 * Reconstruye definiciones de columnas personalizadas SOLO cuando faltan en bulk_config
 * pero siguen en columnOrder. No usa todos los columnKeyAliases (evita columnas duplicadas
 * por IDs históricos huérfanos).
 */
export function reconcileCustomColumns(bulkConfig?: BulkProcessConfig): CustomColumn[] {
    ...
    const legacy = buildLegacyColumnIdToName(bulkConfig, []);
    const rebuilt: CustomColumn[] = [];

    for (const id of orderIds) {
        const name = (legacy[id] || '').trim();
        if (!name) continue;
        rebuilt.push({ id, name, type: 'text' });
    }

    return rebuilt;
}

/** Elimina columnas personalizadas duplicadas por nombre (conserva la primera del orden). */
export function dedupeCustomColumnsByName(customColumns: CustomColumn[] = []): CustomColumn[] {
    const seen = new Set<string>();
    const out: CustomColumn[] = [];
    for (const col of customColumns) {
        if (!col?.id || !col.name?.trim()) continue;
        const key = normalizeColumnNameKey(col.name);
...
/**
 * Corrige layout de tabla corrupto: columnas ocultas en bloque, orden vacío,
 * aliases huérfanos y columnas duplicadas por nombre.
 */
export function sanitizeBulkTableLayout(
    bulkConfig?: BulkProcessConfig
): { config: BulkProcessConfig; fixes: string[] } {
    const fixes: string[] = [];
    const config: BulkProcessConfig = { ...(bulkConfig || {}) };

    let customColumns = dedupeCustomColumnsByName(config.customColumns || []);
    if (customColumns.length !== (config.customColumns || []).length) {
        fixes.push('columnas personalizadas duplicadas por nombre');
        config.customColumns = customColumns;
    }

    const orderCustomIds = (config.columnOrder || [])
        .filter(id => id.startsWith('custom_'))
        .map(id => id.slice('custom_'.length));
    if (orderCustomIds.length > 0 && customColumns.length > 0) {
        const orderIdSet = new Set(orderCustomIds);
        const pruned = customColumns.filter(c => orderIdSet.has(c.id));
        if (pruned.length > 0 && pruned.length < customColumns.length) {
            customColumns = pruned;
            config.customColumns = pruned;
            fixes.push('columnas ajenas al orden original eliminadas');
        }
    }

    if (customColumns.length === 0 && orderCustomIds.length > 0) {
        const rebuilt = reconcileCustomColumns(config);
        if (rebuilt.length > 0) {
            customColumns = rebuilt;
            config.customColumns = rebuilt;
            fixes.push('definiciones de columnas reconstruidas desde columnOrder');
        }
    }

    const allIds = buildAllColumnIds(customColumns);
    let columnOrder = config.columnOrder?.length
        ? migrateBulkColumnOrder(config.columnOrder).filter(id => allIds.includes(id))
        : [];
    const missingFromOrder = allIds.filter(id => !columnOrder.includes(id));
    if (missingFromOrder.length > 0) {
        columnOrder = [...columnOrder, ...missingFromOrder];
    }
    if (!config.columnOrder?.length || columnOrder.length !== config.columnOrder.length) {
        config.columnOrder = columnOrder;
        if (config.columnOrder.length > 0) {
            fixes.push('orden de columnas reparado');
        }
    }

    let hiddenColumns = (config.hiddenColumns || []).filter(id => allIds.includes(id));
    const visibleCount = allIds.filter(id => !hiddenColumns.includes(id)).length;
    if (allIds.length > 0 && visibleCount === 0) {
        hiddenColumns = [];
        fixes.push('todas las columnas estaban ocultas');
    }
    if (hiddenColumns.length !== (config.hiddenColumns || []).length) {
    ...
    for (const [id, name] of Object.entries(config.columnKeyAliases || {})) {
        if (validIds.has(id)) {
            prunedAliases[id] = name;
        }
    }
    if (Object.keys(prunedAliases).length !== Object.keys(config.columnKeyAliases || {}).length) {
        config.columnKeyAliases = prunedAliases;
        fixes.push('alias históricos de columnas depurados');
    }
...
/**
 * Inferencia manual de columnas desde localStorage (solo recuperación explícita).
 * @deprecated Preferir sanitizeBulkTableLayout + Restaurar Excel
 */
export function reconcileCustomColumnsWithLocal(
    processId: string,
    bulkConfig?: BulkProcessConfig
): CustomColumn[] {
    const fromConfig = reconcileCustomColumns(bulkConfig);
    const byId = new Map<string, CustomColumn>();
    for (const col of fromConfig) {
    ...
    const legacy = buildLegacyColumnIdToName(bulkConfig, fromConfig);
    const keysWithData = new Set<string>();
    for (const row of Object.values(localValues)) {
        if (!row) continue;
        for (const [key, val] of Object.entries(row)) {
            if (hasBulkCellValue(val) || val === false) {
    ...
    for (const key of keysWithData) {
        ...
        const legacyName = legacy[key]?.trim();
        if (legacyName) {
            byId.set(key, { id: key, name: legacyName, type: 'text' });
        }
    }

    if (byId.size === fromConfig.length) {
        return fromConfig;
    }
...
/** Resuelve columnas efectivas desde bulk_config (sin inferir aliases huérfanos). */
export function resolveEffectiveCustomColumns(
    _processId: string | undefined,
    bulkConfig: BulkProcessConfig | undefined,
    stateColumns: CustomColumn[] = []
): CustomColumn[] {
    if (stateColumns.length > 0) {
        return stateColumns;
    }
    return dedupeCustomColumnsByName(bulkConfig?.customColumns || reconcileCustomColumns(bulkConfig));
}
