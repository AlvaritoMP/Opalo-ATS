...
-- Opalo ATS: crear y consultar sus envíos
...
-- ---------- worker_handoff_packages — Opalo ATS ----------
...
WITH CHECK (source_app = 'Opalo ATS' AND target_app = 'OpsFlow');
...
USING (source_app = 'Opalo ATS')
WITH CHECK (source_app = 'Opalo ATS');
...
-- ---------- worker_handoff_items — Opalo ATS ----------
...
USING (
    EXISTS (
        ...
        WHERE p.id = package_id AND p.source_app = 'Opalo ATS'
    )
);
