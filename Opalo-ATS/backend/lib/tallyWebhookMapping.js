...
function identityColumnIdFromLabel(label) {
...
function isIdentityCustomColumn(col) {
  return canonicalIdentityMappingKeyFromCustomColumn(col) != null;
}

function migrateIdentityTallyFieldMapping(mapping = {}, customColumns = []) {
  const out = { ...mapping };
...
function identityPartsFromTableColumns(bulkRaw, customColumns) {
  let nombres = '';
  let apellidoPaterno = '';
  let apellidoMaterno = '';
  let dni = '';
...
function applyCanonicalIdentityFromForm(candidate, bulkRaw, customColumns, isBulk) {
