# ⚠️ IMPORTANTE: Configurar Variables de Entorno en EasyPanel

## 🔴 Problema

El build fue exitoso (✓ built in 1m 18s), pero la app no levanta porque **las variables de entorno NO están configuradas** en EasyPanel.

Los logs muestran:
- ✅ Build exitoso
- ✅ Archivos creados en `dist/`
- ✅ Caddy corriendo
- ❌ **App no puede conectarse a Supabase** (falta `VITE_SUPABASE_URL` y `VITE_SUPABASE_ANON_KEY`)
...
- **Nombre**: `VITE_API_URL`
- **Valor**: (La URL de tu backend en producción, por ejemplo: `https://backend-opalo-ats.tu-dominio.com`)
- **Scope**: **Build-time** o **Build & Runtime** ⚠️
...
- Debe mostrar los archivos creados en `dist/`
...
- Si no ves las variables, no están marcadas como "Build-time"

### Verificar Caddy

Si Caddy no está sirviendo archivos correctamente:

1. Verifica que `dist/` existe en el contenedor
2. Verifica que `index.html` existe en `dist/`
3. Verifica que los assets existen en `dist/assets/`
