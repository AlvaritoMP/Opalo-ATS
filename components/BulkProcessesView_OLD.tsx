...
import { Check, X, Loader2, Send, Archive, Search, ChevronDown, ChevronUp, Plus, Edit, Trash2 } from 'lucide-react';
            ...
            {isOpen && (
                <div className="mb-4 space-y-2">
                    ...
                    >
                        ...
                        Enviar a n8n ({selectedIds.length})
                    </button>
                </div>
            )}
            <button
...
export const BulkProcessesView: React.FC<BulkProcessesViewProps> = () => {
    ...
    const loadBulkProcesses = useCallback(async () => {
        ...
        try {
            ...
            // Si hay procesos y no hay uno seleccionado, seleccionar el primero
    ...
    // Cargar procesos masivos al montar
    useEffect(() => {
        loadBulkProcesses();
    }, []);
    ...
    const updateCandidateStatus = useCallback(async (
        ...
        try {
            ...
            // Remover del optimistic update después de éxito
    ...
    const handleBulkApprove = useCallback(async () => {
        ...
        // Encontrar la siguiente etapa (asumiendo que hay una etapa de "Aprobado")
        // Por ahora, solo actualizamos el estado optimísticamente
        ids.forEach(id => {
    ...
    const handleBulkReject = useCallback(async () => {
        ...
        ids.forEach(id => {
            applyOptimisticUpdate(id, { discarded: true });
        });

        try {
            await bulkCandidatesApi.updateCandidatesBatch(ids, {
                discarded: true,
                discardReason: 'Rechazado en proceso masivo',
            });
            setSelectedIds(new Set());
    ...
    const handleBulkArchive = useCallback(async () => {
        ...
        ids.forEach(id => {
            applyOptimisticUpdate(id, { archived: true });
        });
    ...
    const handleWebhook = useCallback(async () => {
        ...
        // TODO: Configurar URL del webhook de n8n en settings
