...
import { Phone, MessageCircle, Loader2, Clock, ChevronDown } from 'lucide-react';
...
export const BulkContactStatusCell: React.FC<BulkContactStatusCellProps> = ({
    ...
    const popoverLeft = Math.min(anchor.x, window.innerWidth - 288);
    const popoverTop = Math.min(anchor.y, window.innerHeight - 360);
    ...
    return (
        ...
        >
            <button
                ...
                onClick={(e) => openPopover('status', e.currentTarget)}
            ...
            {attemptCount > 0 && (
                <button
                    type="button"
                    disabled={disabled}
                    onClick={(e) => openPopover('history', e.currentTarget)}
            ...
            {phone && (
                <button
                    type="button"
                    disabled={disabled || saving}
                    onClick={() => void handleMarkCall('no_answer')}
            ...
            {cooldown && (
                <Clock className="w-3 h-3 text-red-500 shrink-0" aria-hidden />
            )}
            ...
            {popover && (
                <>
                    ...
                    >
                        {popover === 'status' && (
                            <div className="p-2">
                                <p className="text-[10px] uppercase tracking-wide text-gray-500 mb-2 px-1">
                                    Estado de contacto
                                </p>
                                {cooldown && lastAttemptAt && (
                        ...
                        {popover === 'history' && (
                            <div className="p-2 flex flex-col max-h-[320px]">
                                <p className="text-[10px] uppercase tracking-wide text-gray-500 mb-2 px-1">
                                        ...
                                        {history.map((a) => (
                                            ...
                                            >
                                                ...
                                                {a.outcome === 'status_change' && a.notes && (
                                                    <span className="block text-gray-500">{a.notes}</span>
                                                )}
