            ...
            >
                <div className="flex items-start justify-between gap-3 px-4 py-3 border-b border-gray-200 shrink-0">
                    <div className="min-w-0">
                        ...
                        <p className="mt-0.5 text-xs text-gray-500">
                            {isLoading
                                ? 'Cargando En Proceso y Stand By…'
                                : `${entries.length} respuesta${entries.length !== 1 ? 's' : ''} en ${
                                      new Set(entries.map(e => e.processId)).size
                                  } proceso${new Set(entries.map(e => e.processId)).size !== 1 ? 's' : ''}`}
                        </p>
                    </div>
                    <button
                        type="button"
                ...
                <div className="flex-1 overflow-y-auto px-2 py-2">
                    {isLoading && entries.length === 0 ? (
                        <div className="flex flex-col items-center justify-center gap-2 px-2 py-12 text-sm text-gray-500">
                            <Loader2 className="w-5 h-5 animate-spin text-sky-600" />
                            Cargando respuestas de procesos En Proceso y Stand By…
                        </div>
                    ) : entries.length === 0 ? (
                        ...
                        <>
                            {isLoading && (
                                <div className="flex items-center gap-2 px-2 py-2 mb-2 text-xs text-sky-800 bg-sky-50 rounded-md">
